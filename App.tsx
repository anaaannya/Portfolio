        ...
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          <nav className="hidden lg:flex items-center gap-1 bg-green-950/20 border border-green-900 px-3 py-1.5 rounded-xl">
            {menuItems.filter(item => item.view !== 'Contact').map((item) => (
              <button key={item.label} onClick={() => setCurrentView(item.view)} className={`px-6 py-2 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-300 ${currentView === item.view ? 'bg-green-500 text-black' : 'text-green-900 hover:text-green-400'}`}>
                [{item.label}]
              </button>
            ))}
          </nav>
